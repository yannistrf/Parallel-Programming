 ...
 *
 * Compile:  
 *    gcc -g -Wall -fopenmp -o omp_upper_mat_vect_rand_split 
 *          omp_upper_mat_vect_rand_split.c 
 * Run:
 *    ./omp_upper_mat_vect_rand_split <thread_count> <m> <n>
 *
 * Input:
 ...
 *           the entries in A, which is an upper diagonal matrix
 * In args:  m, n
 * Out arg:  A
 */
void Gen_matrix(double A[], int m, int n) {
...
#  pragma omp parallel for num_threads(thread_count)  \
      default(none) private(i, j, temp)  shared(A, x, y, m, n) schedule(runtime)
   for (i = 0; i < m; i++) {
      // Calculate our result in a private variable, to avoid false sharing
      temp = 0.0;
      for (j = 0; j < n-i; j++) {
         temp += A[i*n+j]*x[j];
         // y[i] += A[i*n+j]*x[j];  if we didn't define the temp variable
      }
      // Write only once for each element in the y matrix
      y[i] = temp;
   }
